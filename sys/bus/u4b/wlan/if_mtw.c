...
mtw_attach(device_t self)
{
	...
	for (i = 0; i < 4; i++) {
		sc->txd_fw[i] = (struct mtw_txd_fw *)
		    kmalloc(sizeof(struct mtw_txd_fw),
	...
	ret = lksleep(&sc->fwloading, &sc->sc_lock, 0, "fwload", 3 * hz);
...
mtw_write_ivb(struct mtw_softc *sc, void *buf, uint16_t len)
{
	...
	int error = usbd_do_request_flags(sc->sc_udev, &sc->sc_lock, &req, buf,
	    0, &actlen, 1000);
    ...
    int len)
{
	...
	USETW(req.wIndex, reg);
	USETW(req.wLength, len);
	return (usbd_do_request(sc->sc_udev, &sc->sc_lock, &req,
