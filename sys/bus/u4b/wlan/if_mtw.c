...
static int mtw_load_microcode(void *arg);
...
mtw_attach(device_t self)
{
	...
	error = usbd_transfer_setup(uaa->device, &iface_index, sc->sc_xfer,
	    mtw_config, MTW_N_XFER, sc, &sc->sc_lock);
	if (error) {
	...
	if (ntries == 100) {
		device_printf(sc->sc_dev, "failed reading MAC\n");
		goto detach;
	}
    ...
    const uint8_t mac[IEEE80211_ADDR_LEN])
{
	...
	rvp = kmalloc(sizeof(struct mtw_vap), M_80211_VAP, M_WAITOK | M_ZERO);
	vap = &rvp->vap;
...
mtw_write_ivb(struct mtw_softc *sc, const void *buf, uint16_t len)
{
	...
	int error = usbd_do_request_flags(sc->sc_udev, &sc->sc_lock, &req,
	    __DECONST(void *, buf), 0, &actlen, 1000);
...
mtw_write_cfg(struct mtw_softc *sc, uint16_t reg, uint32_t val)
{
	...
	USETW(req.wLength, 4);
	val = htole32(val);
	error = usbd_do_request(sc->sc_udev, &sc->sc_lock, &req, &val);
	return (error);
}
    ...
    uint32_t offset)
{
	...
	for (;;) {
		...
		memcpy(txd->buf, fw + sent, xferlen);
...
mtw_do_request(struct mtw_softc *sc, struct usb_device_request *req, void *data)
{
	...
	while (ntries--) {
		err = usbd_do_request_flags(sc->sc_udev, &sc->sc_lock, req, data,
...
mtw_write_2(struct mtw_softc *sc, uint16_t reg, uint16_t val)
{
	...
	USETW(req.wIndex, reg);
	USETW(req.wLength, 0);
	return (usbd_do_request(sc->sc_udev, &sc->sc_lock, &req, NULL));
}
...
mtw_write_region_1(struct mtw_softc *sc, uint16_t reg, uint8_t *buf, int len)
{
	...
	USETW(req.wIndex, reg);
	USETW(req.wLength, len);
	return (usbd_do_request(sc->sc_udev, &sc->sc_lock, &req, buf));
}
...
mtw_mcu_cmd(struct mtw_softc *sc, u_int8_t cmd, void *buf, int len)
{
	...
	sc->txd_fw->len = htole16(len);
	sc->txd_fw->flags = htole16(MTW_TXD_CMD | MTW_TXD_MCU |
	    (cmd & 0x1f) << MTW_TXD_CMD_SHIFT | (sc->cmd_seq & 0xf));

	memcpy(sc->txd_fw->buf, buf, len);
	memset(sc->txd_fw->buf + len, 0, MTW_DMA_PAD);
...
{
	return (kmalloc(sizeof(struct mtw_node), M_80211_NODE,
		M_NOWAIT | M_ZERO));
}
static int
...
mtw_bulk_rx_callback(struct usb_xfer *xfer, usb_error_t error)
{
	...
	/* 4 bytes header, 4 bytes padding. */
	while (xferlen > offset + 8) {
		...
		if ((dmalen + 8) > (uint32_t)xferlen - offset) {
			MTW_DPRINTF(sc, MTW_DEBUG_RECV_DESC | MTW_DEBUG_USB,
			    "bad DMA length %u > %d\n",
			    dmalen + 8, xferlen - offset);
			break;
		}

		/*
		 * If it is the last one or a single frame, we can directly
		 * use the mbuf that we have.
		 */
		if (xferlen - offset - dmalen > 8) {
			m1 = m_copym(m, 0, M_COPYALL, M_WAITOK);
		} else {
			m1 = m;
			m = NULL;
		}
		m1->m_data = (void *)&pkt[1];
		m1->m_pkthdr.len = m1->m_len = dmalen + MTW_DMA_PAD;
		/* Frees the mbuf itself if RX fails. */
		mtw_rx_frame(sc, m1, dmalen);

		offset += dmalen + sizeof(struct mtw_rxd) + MTW_DMA_PAD;
	}
...
mtw_fw_callback(struct usb_xfer *xfer, usb_error_t error)
{
	...
	case USB_ST_SETUP:
		len = data->len + sizeof(struct mtw_txd) + MTW_DMA_PAD;
		usbd_xfer_set_frame_len(xfer, 0, len);
		usbd_xfer_set_frame_data(xfer, 0, sc->txd_fw, len);
		usbd_transfer_submit(xfer);
		break;
...
static void
mtw_stop(void *arg, int disable)
{
	struct mtw_softc *sc = (struct mtw_softc *)arg;
	uint32_t tmp;
	int i, ntries, error, qid;
...
static driver_t mtw_driver = {
	.name = "mtw",
	.methods = mtw_methods,
	.size = sizeof(struct mtw_softc),
};

static devclass_t mtw_devclass;

DRIVER_MODULE(mtw, uhub, mtw_driver, mtw_devclass, mtw_driver_loaded, NULL);
MODULE_DEPEND(mtw, wlan, 1, 1, 1);
MODULE_DEPEND(mtw, usb, 1, 1, 1);
MODULE_DEPEND(mtw, firmware, 1, 1, 1);
#if 0   /* Not implemented by DragonFly */
