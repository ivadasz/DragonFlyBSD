...
#define MTW_LOCK_INIT(sc) \
	lockinit(&(sc)->sc_lock, device_get_nameunit((sc)->sc_dev), \
	    0, LK_CANRECURSE)
#define	MTW_LOCK(sc)		lockmgr(&(sc)->sc_lock, LK_EXCLUSIVE)
#define MTW_UNLOCK(sc)		lockmgr(&(sc)->sc_lock, LK_RELEASE)
#define	MTW_LOCK_ASSERT(sc)	KKASSERT(lockstatus(&(sc)->sc_lock, curthread) == LK_EXCLUSIVE)
#define MTW_LOCK_DESTROY(sc)	lockuninit(&(sc)->sc_lock)
